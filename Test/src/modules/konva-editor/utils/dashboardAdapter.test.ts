...
describe('convertDashboardRoomToDocument', () => {
  it('converts beds/walls/texts with status mapping', () => {
    ...
    const bed = doc.elementsById.b1 as any
    ...
    const wall = doc.elementsById.w1 as any
    expect(wall.t).toBe('line')
    expect(wall.strokeW).toBe(8)

    const text = doc.elementsById.t1 as any
    expect(text.t).toBe('text')
    expect(text.align).toBe('l')

    expect((doc.elementsById.t2 as any).align).toBe('c')
    expect((doc.elementsById.t3 as any).align).toBe('r')
  })
  ...
  it('handles missing walls/texts and defaults', () => {
    ...
    const bed = doc.elementsById.b2 as any
  ...
  it('uses default wall/text styles when fields are missing', () => {
    const doc = convertDashboardRoomToDocument({
      room: {
        ...
        texts: [{ id: 't', text: 'T', x: 0, y: 0, fontSize: undefined, color: undefined } as any],
      },
      statuses: [],
    } as any)

    expect((doc.elementsById.w as any).stroke).toBe('#000000')
    expect((doc.elementsById.w as any).strokeW).toBe(6)
    expect((doc.elementsById.t as any).fontSize).toBe(16)
    expect((doc.elementsById.t as any).fill).toBe('#000000')
  })
})
