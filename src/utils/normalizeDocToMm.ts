...
import { roundTo, toMm } from '@/utils/units'

export interface NormalizeDocOptions extends UnitOptions {
...
function normalizeMarginLike(m: unknown, unit: LengthUnit, options: UnitOptions): Margin | undefined {
...
function normalizeSurface(surface: Surface, unit: LengthUnit, options: UnitOptions): Surface {
  return {
    ...surface,
    w: toMm(surface.w, unit, options),
    h: toMm(surface.h, unit, options),
    margin: normalizeMarginLike(surface.margin, unit, options) ?? surface.margin,
  }
}

function normalizeNode(node: UnifiedNode, unit: LengthUnit, options: UnitOptions): UnifiedNode {
  if (node.t === 'line') {
    return {
      ...node,
...
export function normalizeDocToMm(input: unknown, options: NormalizeDocOptions = {}): Doc {
  if (!isRecord(input)) {
    throw new Error('normalizeDocToMm: input must be an object')
  }
  ...
  if (!Array.isArray(doc.surfaces) || !Array.isArray(doc.nodes)) {
    throw new Error('normalizeDocToMm: invalid doc shape')
  }

  const normalized: Doc = {
    ...doc,
    unit: 'mm',
    surfaces: doc.surfaces.map((s) => normalizeSurface(s, unit, options)),
    nodes: doc.nodes.map((n) => normalizeNode(n, unit, options)),
  }
