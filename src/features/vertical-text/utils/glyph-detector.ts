 ...
 * 句読点かどうかを判定
 */
export const isPunctuation = (char: string): boolean => PUNCTUATION.includes(char)

/**
 * 右上寄せが必要な文字かどうか（句読点 + 小書き仮名）
 */
export const isTopRightAlign = (char: string): boolean => isSmallKana(char) || isPunctuation(char)
 ...
 * 開き括弧かどうか
 */
export const isOpeningBracket = (char: string): boolean => OPENING_BRACKETS.includes(char)

/**
 * 閉じ括弧かどうか
 */
export const isClosingBracket = (char: string): boolean => CLOSING_BRACKETS.includes(char)
