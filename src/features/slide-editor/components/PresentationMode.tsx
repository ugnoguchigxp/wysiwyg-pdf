...
import { X, ChevronLeft, ChevronRight, Maximize, Minimize, Play } from 'lucide-react'
...
export const PresentationMode: React.FC<PresentationModeProps> = ({
    ...
    return (
        ...
        >
            ...
            {/* Top Right Controls */}
            <div className="absolute top-4 right-4 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity z-20" onClick={e => e.stopPropagation()}>
                {isFullscreen ? (
                    <button onClick={toggleFullscreen} className="p-2 bg-black/50 text-white rounded-full hover:bg-white/20" title="Exit Fullscreen">
                        <Minimize size={20} />
                    </button>
                ) : (
                    <button onClick={toggleFullscreen} className="p-2 bg-black/50 text-white rounded-full hover:bg-white/20" title="Enter Fullscreen">
                        <Maximize size={20} />
                    </button>
                )}
                {/* On Exit: only if not fullscreen? Or always? */}
                {/* User seems to want Exit from component entirely */}
                <button onClick={onExit} className="p-2 bg-black/50 text-white rounded-full hover:bg-white/20 hover:text-red-400" title="Close">
                    <X size={20} />
                </button>
            </div>

            {/* Floating Controls (Bottom) */}
            <div className="absolute bottom-6 left-1/2 -translate-x-1/2 flex items-center gap-4 opacity-0 group-hover:opacity-100 transition-opacity bg-black/60 p-2.5 rounded-full border border-white/10 z-20 backdrop-blur-sm" onClick={e => e.stopPropagation()}>
                <button onClick={prevSlide} className="text-white hover:text-primary transition-colors disabled:opacity-30" disabled={currentIndex === 0}>
                    <ChevronLeft size={24} />
                </button>
                <span className="text-white text-sm font-medium min-w-[3rem] text-center select-none font-mono">
                    {currentIndex + 1} / {slides.length}
                </span>
                <button onClick={nextSlide} className="text-white hover:text-primary transition-colors disabled:opacity-30" disabled={currentIndex === slides.length - 1}>
