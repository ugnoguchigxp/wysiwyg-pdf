...
const SlideThumbnail: React.FC<SlideThumbnailProps> = ({
  ...
  const slidePreview = React.useMemo(() => {
    ...
    const masterSurface = slide.masterId ? doc.surfaces.find((s) => s.id === slide.masterId) : null
    const masterNodes = masterSurface ? doc.nodes.filter((n) => n.s === masterSurface.id) : []
