...
import type { UnifiedNode, TextNode, ShapeNode } from '@/types/canvas'
import { ptToMm } from '@/utils/units'

export type LayoutType = 'blank' | 'title' | 'title-only' | 'title-content' | 'two-column' | 'three-column' | 'four-images' | 'big-number'
...
const Rect = ({ x, y, w, h, fill = '#cbd5e1' }: { x: string, y: string, w: string, h: string, fill?: string }) => (
    <div className="absolute" style={{ left: x, top: y, width: w, height: h, backgroundColor: fill }} />
)
...
export const SLIDE_LAYOUTS: LayoutDefinition[] = [
    {
        ...
        generateNodes: (s, w, h) => {
            ...
            const subY = h / 2 + 15
            return [
                createText(s, 'Click to add Title', SECTION_MARGIN, titleY, w - SECTION_MARGIN * 2, 35, 44, 'c', 700),
                createText(s, 'Click to add Subtitle', SECTION_MARGIN, subY, w - SECTION_MARGIN * 2, 20, 24, 'c', 400),
            ]
        },
    },
    {
        ...
        generateNodes: (s, w) => [
            createText(s, 'Click to add Title', SECTION_MARGIN, TITLE_Y, w - SECTION_MARGIN * 2, TITLE_H, 36, 'c', 700),
        ],
    },
    {
        id: 'title-content',
        ...
        generateNodes: (s, w, h) => [
            createText(s, 'Click to add Title', SECTION_MARGIN, TITLE_Y, w - SECTION_MARGIN * 2, TITLE_H, 32, 'l', 700),
            createText(s, '• Click to add text\n• Second point', SECTION_MARGIN, CONTENT_Y, w - SECTION_MARGIN * 2, h - CONTENT_Y - SECTION_MARGIN, 18, 'l'),
        ],
    },
    {
        id: 'two-column',
        ...
        generateNodes: (s, w, h) => {
            ...
            return [
                createText(s, 'Click to add Title', SECTION_MARGIN, TITLE_Y, w - SECTION_MARGIN * 2, TITLE_H, 32, 'l', 700),
                createText(s, '• Bullet point 1', SECTION_MARGIN, CONTENT_Y, colW, h - CONTENT_Y - SECTION_MARGIN, 18, 'l'),
                createPlaceholder(s, SECTION_MARGIN + colW + 10, CONTENT_Y, colW, h - CONTENT_Y - SECTION_MARGIN),
            ]
        },
    },
    {
        ...
        generateNodes: (s, w, h) => {
            ...
            return [
                createText(s, 'Click to add Title', SECTION_MARGIN, TITLE_Y, w - SECTION_MARGIN * 2, TITLE_H, 32, 'c', 700),
    ...
    },
    {
        ...
        icon: (
            <IconWrapper>
                <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-2xl font-bold text-primary/80">#</div>
