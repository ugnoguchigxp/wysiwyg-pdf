...
import { Group, Line as KonvaLine, Rect, Text } from 'react-konva'
...
export const SpeechBubbleRenderer: React.FC<SpeechBubbleRendererProps> = ({
  ...
  const renderBubbleShape = () => {
    switch (shapeType) {
      ...
      case 'custom':
        return <KonvaLine points={element.pathPoints || []} closed={true} {...bubbleShapeProps} />
      default:
        return null
    }
  }
  ...
  return (
    <Group {...commonProps}>
      ...
      {!isEditing && displayText && (
        ...
        >
          {vertical ? (
            <VerticalKonvaText node={verticalTextProps} visible={!element.hidden} />
          ) : (
            <Text
              ref={textRef}
              x={textX}
