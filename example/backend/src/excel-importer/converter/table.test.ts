...
import type { ExcelRow, ExcelSheet } from '../types/excel'
...
describe('convertToTableNode', () => {
  ...
  test('handles merged cells', () => {
    const rows = [createMockRow(0, 15, [createMockCell(0, 0, 'Merged')])]
    const mergedCells = [{ startRow: 0, startCol: 0, endRow: 0, endCol: 2 }]
  ...
  test('generates unique id for table', () => {
    ...
    const result1 = convertToTableNode(
      sheet,
      range,
      rowHeights,
      colWidths,
    ...
    const result2 = convertToTableNode(
      sheet,
      range,
      rowHeights,
      colWidths,
  ...
  test('calculates correct dimensions', () => {
    const rows = [createMockRow(0, 10, []), createMockRow(1, 20, []), createMockRow(2, 30, [])]
    const sheet = createMockSheet(rows)
  ...
  test('handles empty rows', () => {
    const rows = [createMockRow(0, 15, []), createMockRow(1, 20, [])]
    const sheet = createMockSheet(rows)
