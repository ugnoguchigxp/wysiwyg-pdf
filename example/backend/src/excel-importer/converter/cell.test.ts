import { convertCellStyle, borderWidth } from '../converter/style';
...
describe('convertCell', () => {
    test('converts cell with string value', () => {
        const cell = { row: 0, col: 0, value: 'test', style: {} };
        const result = convertCell(cell, DEFAULT_IMPORT_OPTIONS);
        expect(result.v).toBe('test');
    });

    test('converts cell with number value', () => {
        const cell = { row: 0, col: 0, value: 123.45, style: {} };
        const result = convertCell(cell, DEFAULT_IMPORT_OPTIONS);
        expect(result.v).toBe('123.45');
    });

    test('converts cell with boolean value', () => {
        const cell = { row: 0, col: 0, value: true, style: {} };
        const result = convertCell(cell, DEFAULT_IMPORT_OPTIONS);
        expect(result.v).toBe('TRUE');
    });

    test('converts cell with date value', () => {
        const cell = { row: 0, col: 0, value: new Date('2024-01-01T00:00:00Z'), style: {} };
        const result = convertCell(cell, DEFAULT_IMPORT_OPTIONS);
        expect(result.v).toContain('2024');
    });

    test('converts cell with rich text', () => {
        const cell = {
            row: 0, col: 0,
            value: {
                richText: [
                    { text: 'Hello', font: { name: 'Arial', size: 11 } },
                    { text: 'World', font: { name: 'Arial', size: 11, bold: true } }
                ]
            },
            style: {}
        };
        const result = convertCell(cell, DEFAULT_IMPORT_OPTIONS);
        expect(result.v).toBe('HelloWorld');
        expect(result.richText).toHaveLength(2);
        expect(result.richText?.[1].bold).toBe(true);
    });

    test('preserves formula', () => {
        const cell = { row: 0, col: 0, value: 10, formula: 'SUM(A1:A5)', style: {} };
